...
2026-09-13 动态视觉补充：输入时仅将越界光标移入本地可视区，不得无条件滚到共享画布底部而隐藏 Bash 开头几行；后台光标变化不得抢走用户正在查看的历史。小屏字号保持本地宽度预算独立，大屏客户端加入不得缩小本地字号。控制连接反复建立/断开还必须保持实际 tmux server/pane 身份；测试依赖已隔离为 3.7c，禁止用同名新建会话冒充会话保留。当前实现候选 `570df98`，完整验收仍未通过，Claude 会话恢复及全矩阵重跑未完成。
...
2026-09-13 本端显示窗口候选：桌面字号仅由本端偏好、宽度/DPR 和统一 104 列预算决定，不以当前远端行数重新缩放；桌面请求列数不超过 104，行数按本端实际高度计算。服务端精确 grid 保持不变，本地窗口可滚动查看超出行列，输入/首次进入跟随底部；本地滚轮按三行消费到边缘后接续原生历史。该候选替代“一次性缩放全部服务端行”的行为，但不允许无滚动通路的裁切、遗漏 statusline 或宽度溢出遮挡；必须实际验收，不因单测通过生效为已验收基线。

2026-09-13 补充强制验收：本端 Panel 尺寸/DPR 不变时，另一端进入同一会话不能导致本端字号下降，比较不能排除服务端网格变化。页面普通刷新及强制刷新后必须完整重跑全部动态视觉场景；覆盖大小屏分别刷新、双端同会话时仅刷新一端，刷新前 PASS 不可替代刷新后证据。

2026-09-13 用户反馈回归：Panel 拖动/切换和跨大小屏使用不得销毁重建已有终端或重复播放初始化字号；实际几何变化应重新申请客户端网格，绘制仍使用服务端确认的精确网格。大屏铺满、小屏尽可能铺满是独立视觉验收项，不可用“未截断”替代。每个版本必须执行 `docs/qa/terminal-visual-release-checklist.md` 的动态场景矩阵，未测项不得记 PASS；本要求覆盖此前仅最终截图的局部验收。

2026-09-13 回归修复与追加需求：桌面字号通过离屏测量选择最终值；同一尺寸、DPR、渲染器和服务端网格再次进入视野不能重复播放试字号过程。桌面 SSH 与 CLI 均遵循服务端实际网格，并在 tmux layout-change 时先通知新网格再输出重绘数据，避免 readline 按不同列宽回写。本条覆盖此前 SSH 自适应本地列数的规则；不同宽高比例下允许留白，不以错误列数换取铺满。同一位置内的 Panel 标签支持鼠标按住拖动，目标标签左右半区分别插入其前后，末端空白区可放到最后；保留会话 ID、编号、活动标签及现有持久化路径，不关闭或重建会话。手机原有横向触摸滑动保持不变，触摸长按排序尚未纳入本次实现。

紧凑桌面 CLI 余量优化：最大可容纳字号及精确服务端 grid 保持不变，仅当底部余量足够且保留 1px 安全边距时，允许每行增加最多 1 个物理像素间距；不改变字形、横向字距、行列数或历史步进。最终实际画布若超界则恢复原行高。大屏、SSH 和手机不应用此修正。余量不足一个完整网格的单像素行高增量时保持原样，不能声称消除了所有留白。

2026-09-12 工具栏与文件列表：工作区 Tab 按钮置于工具栏第二位，其余模块顺延。独立文件按钮控制随当前 SSH Panel 的文件列表，默认收起，移到终端左侧/工具栏右侧区域；删除最右侧 20px 展开条，左侧文件栏从右边缘拖动调宽。文件列表显示状态仅本地内存，不改变当前模块或关闭终端。整页 `.terminal-grid` 滚动条改为深绿滑块 `#355f3d` / 浅绿轨道 `#d9edd9`，终端内部滚动条和步进不变。

2026-09-12 顶部工作区 Tab 栏默认收起：左侧工具栏提供键盘/鼠标/触摸可操作的展开/收起按钮，含 aria-expanded 和中英文标签；折叠时不占行高。桌面额外释放 30px、手机 38px。刷新恢复默认收起，仅本浏览器内存状态，不广播、不持久化、不关闭终端、不切换当前工作区。Panel 标题栏及 CLI statusline 不隐藏。展开后保留工作区选择、创建、重命名和关闭保护。

2026-09-12 Panel 标题栏：桌面精细指针环境从 36px 收紧到 28px，标签高度 24px、行高 20px、中文原字号不变；新增按钮 24px，不改变关闭确认、重命名或横向滚动语义。手机触摸环境仍保留原尺寸。每个桌面 Panel 增加 8px 终端高度。

2026-09-12 用户追加空间优化：桌面精细指针环境的顶部工作区 Tab 条由 38px 收紧至 30px；移除网页全局底部状态条（26px），保留终端内部 Claude statusline。手机顶部触摸目标不缩小。桌面纵向可用空间增加约 34px；双行布局平均每行增加约 17px。

2026-09-12 Claude 完整画布补充（用户确认全部内容可见后要求尽可能铺满）：全屏 CLI 必须保留服务端宣布的精确行列，不能通过减少本地行列填满空间。保持自然字距和字形比例，以实际渲染尺寸寻找可完整容纳网格的最大字号，滚动条前保留 2px、底部保留 1px。若终端网格与 Panel 宽高比例不同，允许未受约束方向留白，不允许拉伸字形、扩大中文间距或截断输入区/statusline 来消除留白。此条优先于下述通用桌面自适应列数规则；SSH 与手机既有规则不变。
...
- 桌面宽度小于 3000 CSS px 时，一行最多 2 个 Panel、首屏最多 2 行（2026-09-12 用户批准由 3×2 调整为 2×2）；第 5 个及以后 Panel 位于后续纵向页面。已观察的 1920×1080 与 2860×988 均属于该档。3440×1440 保持 4×2，不受紧凑布局覆盖。手机布局不变。增加宽度用于提高可容纳字号，不承诺受高度限制时与大屏字号完全相同。
