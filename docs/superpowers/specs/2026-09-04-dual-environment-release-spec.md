...
- 快照保留相同的 `user_id`、`connection_id` 和 tab `terminal_id`。tmux 名仍由这三个值确定，因此测试入口能 attach 到生产既有 tmux 会话。
- release-test 使用 `-preserve-terminal-sessions`。在测试 UI 关闭标签页只修改测试布局，不发送远端 `tmux kill-session`。
- release-test 内的键盘输入、clear-history、CLI 操作仍会作用于共享 tmux；这是“复用同一交互会话”的必然结果，验收人员必须按真实共享会话对待。
