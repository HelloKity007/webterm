...
2026-09-13 当前视觉阻断已关闭：测试 tmux 使用私有 3.7c 和 `webterm-release-test-fixed` socket，400 次控制连接压力测试通过；候选 `157373e` 在 9444 完成 fresh/normal/hard 三轮 33 项动态矩阵，OpenCLI 实际 Chrome、双屏录像抽样和移动横竖屏证据通过。物理手机复检仍须与浏览器模拟区分；生产未修改。详见 `docs/qa/2026-09-13-workspace-close-acceptance.md`。
...
- [x] 2026-09-13 本端字号与远端 grid 解耦：`157373e` 保持精确 ANSI 行列，用本地滚动窗口容纳超出部分；2860 中间宽度按完整 104 列搜索最大本地字号，实际 Chrome 右侧余量约 17px。输入跟随、三行滚动、双屏及三轮刷新矩阵通过。
