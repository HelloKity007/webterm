...
2026-09-13 增量：用户已批准工作区 Tab 关闭与下属会话级联终止，仅发布9444。实现共享关闭按钮、带数量/不可撤销警告的确认、最后工作区替换为空工作区、失败重试和旧连接重建防护；用独立临时测试账号的专用会话验收，禁止关闭现有用户工作区来测试“最后一个”场景。验收入口 `scripts/verify-workspace-close.mjs`；原动态视觉门禁继续保留，不将本功能通过等同全版本验收通过。

2026-09-13 当前阻断项：测试 tmux 3.5a 曾崩溃丢失 Claude 进程；已用私有二进制 3.7c 和 `webterm-release-test-fixed` socket 隔离，400 次控制连接压力测试通过，未修改系统 tmux/生产。`570df98` 修复输入视口强制到底部的问题。继续完成首次加载/普通刷新/强刷三轮动态视觉、恢复 Claude 验收会话、保存截图和动态证据；此前部分 PASS 不代表当前候选全量通过。详见 `docs/qa/2026-09-13-tmux-control-crash.md`。
...
- [ ] 2026-09-13 本端字号与远端 grid 解耦候选：桌面保持本端偏好字号、精确 ANSI 行列，用本地滚动窗口容纳超出部分；输入跟随底部，滚轮优先按三行移动本地窗口，到边缘后进入原生历史。新增单测通过后仅测试部署，必须重跑双屏及刷新矩阵；不得在未验证时宣称完成。

- [ ] 同会话跨屏字号回归已复现 FAIL（`docs/qa/2026-09-13-peer-font-regression.md`）；修复后须在普通刷新、强制刷新后分别重跑完整动态视觉矩阵，包含另一端未刷新的双屏同会话场景。不得以加载成功或刷新前结果代替。
...
- [x] 紧凑 CLI 最大字号后的物理像素余量：每行最多追加 1 个物理像素，仅可完整容纳时应用，保留 1px 底部余量并在真实渲染溢出时回退；不改变服务端 grid、中文横向字距、大屏或手机。需如实报告未能缩减的比例留白。

- [x] 工作区 Tab 按钮排第二；SSH 文件列表默认关闭、从右移左、工具栏独立按钮控制，去掉右侧展开窄条；整页滚动条提高配色对比度，不改变终端历史滚动。

- [x] 顶部工作区 Tab 栏默认收起，左侧工具栏提供展开入口；独立本地 UI 状态避免触发布局广播。验收展开/收起高度恢复、工作区操作保留、Claude 历史往返与手机回归，仅测试发布。

- [x] Panel 标题栏桌面 36→28px，中文字号不变；标签行高 20px，手机原触摸尺寸不变。验收文字边界、按钮、名称栏滚动以及终端底部显示。

- [x] 用户追加：桌面顶部工作区 Tab 条 38→30px，移除网页底部状态条，保留 CLI statusline 和手机顶部触摸目标；随 2×2 测试候选共同回归。

2026-09-12 Claude 追加切片：用户确认完整内容已恢复；下一步保留精确服务端 grid，在自然字距下搜索最大可容纳字号，减少原单向缩小产生的多余空白。验收 1920/2860/3440 的输入区、statusline、历史往返和边界，不以裁切或拉伸填满。仅测试 9444，物理屏幕由用户复检。

2026-09-12 用户复检纠正：前轮约 -0.9px 的重叠不符合要求。`abffcf1` 改为实际滚动条边界/DPR 驱动的完整列网格。opencli 真实 Chrome（DPR 1.203125）测得余量 0.08–0.09px；四组有界面 Chrome 检查余量 0–1.68px、无重叠及高度裁切，证据 `runtime/terminal-width-qa/`，回归脚本 `scripts/verify-terminal-width.mjs`。这些是本项几何验证，不代表全部 CLI 历史与真机验收。

- [x] 小于 3000 CSS px 的桌面重排为 2 列（2026-09-12 用户批准从 3×2 改为 2×2）；两行占满首屏，第 5 个及以后 Panel 通过页面纵向滚动访问；3440×1440 保持 4×2，手机不变。
- [x] 有横向溢出的 Panel 名称栏接管并消费滚轮；无溢出名称栏仍可驱动页面纵向滚动。
- [x] 终端滚动条贴 Panel 右边缘；1920×1080 的 76×19 字符网格与滚动条左侧像素差为 0、横向 overflow 为 0；2860×988 的 79×19 差值约 0.9px（设备像素舍入）、overflow 为 0；3440×1440 的 104×31 差值 0.75px、overflow 为 0。
- [x] 真实 Chrome/opencli 保存 1920、2860、3440、第三行和移动 viewport 证据；名称栏横移 37px 时页面保持 0，随后页面下滚至 509 并显示 Panel 7。
- [x] UI 26 files / 101 tests、lint、production build、Go tests 与 9444 release-deploy 门禁通过；手机 6 项 smoke 通过。真机、远程 CLI 5000 行压力、完整 M3–M8 仍按 NOT RUN/既有阶段管理。
...
- [2026-09-12] dev-1.0.4 完成紧凑桌面 3×2 首屏、额外 Panel 纵向页面、名称栏滚轮隔离和滚动条边界适配。1920/2860/3440 实际浏览器像素与滚动行为通过；移动 smoke 未回退。仅部署 9444，生产 9443 保持 `2df868a`。
