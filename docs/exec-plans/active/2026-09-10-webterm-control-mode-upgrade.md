...
> 2026-09-11状态更新：生产基线`2df868a`用户已确认，后续开发在主仓库`dev-1.0.4`。本文件是补充计划；最新产品约束以主Spec v1.2的P0-MOB和主开发计划M-MOB为准。
>
> 重要澄清：现有Control Mode传输与手机文本阅读层不等于独立Claude transcript/replay服务。手机通过有节流的原生SGR滚动访问应用保留历史，可能改变同一共享TUI画面；阅读层默认换行并保留“终端输入/换行阅读”，不再放“更早输出/更新输出”按钮。独立应用回放、真实只读权限及所有真机兼容仍不能标为完成。下文2026-09-10记录保留为历史，不覆盖上述最新事实。
